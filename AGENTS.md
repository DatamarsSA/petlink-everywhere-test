...
# Petlink Everywhere Test - Cursor Rules (Concise)
...
- **`subscriptions-manager`**: Payment processing and subscription lifecycle (Chargebee).
...
Tests are organized by actor (`user/`, `cct/`) and journey (`registration/`, `subscriptions/`).

---

## 3. How to Write a Test: A Practical Guide
...
## 4. Core Principles & TypeScript Usage

- **Clients are the Interface**: Interact with the backend **only** through the provided clients (`petlink`, `testHelper`). The client abstracts away authentication, performance tracking, and protocol details. The pattern is `petlink.<service>.<protocol>.<auth>.<endpoint>`.
  - `petlink.core.graphql.authJwt.getUser()`
  - `petlink.core.graphql.public.signUp()`
...
- **Logging Strategy**:
  - **`logger.debug()`** (clients only): Low-level flow (auth, caching, connection state). Use to diagnose client/wrapper issues.
  - **`logger.error()`** (clients only): Actual failures (catch blocks, timeouts, validation errors).
  - **`logger.info()`** (tests only): Business flow with full context—log payloads before API calls, responses after. Default run shows only test flow; enable debug to diagnose client issues; errors always visible.
  
  ```typescript
  // Client (debug + error only)
  try {
    logger.debug("[WS] Subscription starting", { subId, operationName });
  } catch (error: any) {
    logger.error("[WS] Subscription failed", { error: error.message, subId });
  }
  
  // Test (info only)
  logger.info("▶ Starting pet creation flow");
  logger.info(`→ Calling createPet:`, dogPayload);
  const response = await petlink.core.graphql.authJwt.createPet(dogPayload);
  logger.info(`← Response:`, response);
  ```

---

## 5. Anti-Patterns to Avoid (Top 5)

1.  **❌ Don't use spread operators (`...fixtures`) for payloads.** Always define fields explicitly.
2.  **❌ Don't create opaque assertion helpers.** All `expect` statements must be visible and explicit within the test. Clarity over DRYness.
...
- **Format**: Use standard GraphQL syntax inside these files.
