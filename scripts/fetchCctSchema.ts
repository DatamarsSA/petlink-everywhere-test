...
const apiKeyVarName = `CCT_GRAPHQL_API_KEY_${branchName}`;
const apiUrlVarName = `CCT_GRAPHQL_API_URL_${branchName}`;

const CCT_GRAPHQL_API_KEY = process.env[apiKeyVarName];
const CCT_GRAPHQL_API_URL = process.env[apiUrlVarName];
if (
  !CCT_GRAPHQL_API_KEY ||
  !CCT_GRAPHQL_API_URL
) {
...
const SCHEMA_GRAPHQL_PATH = './schema/graphql/cct_schema.graphql';
...
try {
  execSync(
    `${getBinFile('get-graphql-schema')} -h "X-API-KEY=${CCT_GRAPHQL_API_KEY}" ${CCT_GRAPHQL_API_URL} > ${SCHEMA_GRAPHQL_PATH}`
  );
