...
export async function fetchSchemas(): Promise<void> {
  ...
  execSync(`npx get-graphql-schema -h "X-API-KEY=${process.env.CORE_GRAPHQL_API_KEY}" ${process.env.CORE_GRAPHQL_API_URL} > ${SCHEMA_DIR}/core_schema.graphql`, {
    stdio: "inherit",
  });

  // Fetch CCT
  console.log("  → Fetching CCT schema...");
  execSync(`npx get-graphql-schema -h "X-API-KEY=${process.env.CCT_GRAPHQL_API_KEY}" ${process.env.CCT_GRAPHQL_API_URL} > ${SCHEMA_DIR}/cct_schema.graphql`, {
    stdio: "inherit",
  });
