...
describe.runIf(fxt.isKippyRun).skip("End of Life (EOL) Tests", () => {
  describe("Flow 1: Device-Only Replacement (Active Long Subscription)", () => {
    ...
    beforeAll(async () => {
      ...
      setup = await testHelper
        ...
        .withDog({ gps: { model: DeviceTypeEnum.Evo } })
        .build();
      ...
      await testHelper.purchaseSubscription(setup.user!, setup.dog!.devices.gps!, [longPricing.id]);
    ...
    it("Step 2: Device Selection - Should transition to SHIPPING_INFO_FROM_ONLY_DEVICE", async () => {
      const initRes = await petlink.core.graphqlHttp.authJwt.updateEndOfLife({
        ...
        input: {
          productId,
          userId: testUser.id,
          serialNumber: setup.dog!.devices.gps!.serialNumber,
  ...
  describe("Flow 2: Device + Subscription Replacement (Expired/No Subscription)", () => {
    ...
    it("Step 5: Purchase Subscription (Simulated)", async () => {
      ...
      await testHelper.purchaseSubscription(testUser, setup.dog!.devices.gps!, [selectedPricing.id]);
    });
